...
public class personnel_Manager extends javax.swing.JFrame {
    ...
    public personnel_Manager() {
        initComponents();
        tableModel = (DefaultTableModel) informationPersonner.getModel();
        showALL();
        Label_Admin.setText(login_admin.Name_admin);
        checkValueSearch();
    }
    
    public void checkValueSearch() {
        String input_Search = textField_Search.getText();
    ...
    public void showALL() {
        personnelList = personnelController.findAll();
        tableModel.setRowCount(0);
        for (personnel personnel : personnelList) {
            tableModel.addRow(new Object[]{
                personnel.getId_QuanLy(),
                personnel.getTenQuanLy(),
                personnel.getSoDienThoai(),
                personnel.getDiaChi(),
                personnel.getNgaySinh()
            });
        }
        
        TableRowSorter<DefaultTableModel> sorter = new TableRowSorter<>(tableModel);
        informationPersonner.setRowSorter(sorter);
    }
    ...
    private void initComponents() {
        ...
        informationPersonner = new Design_Item.Table();
        ...
        informationPersonner.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
            ...
            new String [] {
                "ID Nhân viên", "Tên nhân viên", "Số điện thoại", "Địa chỉ", "Ngày sinh"
            }
        ) {
        ...
        informationPersonner.setShowGrid(false);
        informationPersonner.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                informationPersonnerMouseClicked(evt);
            }
        });
        jScrollPane1.setViewportView(informationPersonner);
    ...
    private void informationPersonnerMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_informationPersonnerMouseClicked
        int id_QuanLy = 0;
        String tenQuanLy = null;
        String soDienThoai = null;
        String diaChi = null;
        String ngaySinh = null;
        int indexslect = informationPersonner.getSelectedRow();
        if (indexslect >= 0) {
            personnel PR = personnelList.get(indexslect);
            id_QuanLy = PR.getId_QuanLy();
            tenQuanLy = PR.getTenQuanLy();
            soDienThoai = PR.getSoDienThoai();
            diaChi = PR.getDiaChi();
            ngaySinh = PR.getNgaySinh();
        }
        new edit_Personnel(id_QuanLy, tenQuanLy, soDienThoai, diaChi, ngaySinh).setVisible(true);
        this.dispose();
    }//GEN-LAST:event_informationPersonnerMouseClicked
    ...
    private Design_Item.Table informationPersonner;
